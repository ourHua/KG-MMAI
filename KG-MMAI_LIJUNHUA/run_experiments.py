#!/usr/bin/env python3
"""Run the KG-MMAI manuscript workflow.

The default path uses only files included in the public release.  Use
``--full-local`` when an authorised ``data/train.txt`` is available and the
source-level S0/S1/S2 sensitivity analysis should be rebuilt.
"""
...
SENSITIVITY_GRAPHS = {
    ...
    "S1": "results/sensitivity/edges_S1_adjudicated.csv",
...
def run_command(name, description, command):
    ...
    if completed.returncode == 0:
        ...
        return True
    print(f"[{name}] FAILED ({completed.returncode}) after {elapsed:.1f} s", file=sys.stderr)
    return False
...
def verify_figures(stems=MANUSCRIPT_FIGURES):
    ...
    for stem in stems:
        ...
        print(("MISS " if bad else "OK   ") + stem + suffix)
    if missing:
        print(f"[figure-check] {len(missing)} required asset(s) missing", file=sys.stderr)
        return False
    print("[figure-check] verified 10 manuscript figures / 20 files")
    return True
...
def revision_workflow(full_local=False, continue_on_error=False):
    ...
    if full_local or raw.is_file():
        ...
        print("\n[annotation-sensitivity] skipped: raw corpus is not part of the public release")
...
def public_workflow(continue_on_error=False):
    # Script 08 needs the source corpus, so the public path uses the archived
    # manuscript-reference tables when Figure 6 and the revision audit run.
    return run_sequence(
        (
            "structure",
            "link-prediction",
...
def main():
    ...
    if args.steps:
        ...
        ok = run_sequence(
            ("figures-structure", "figures-results", "figures-revision", "figure-design", "figure-formats"),
            args.continue_on_error,
        )
        needs_check = True
    elif args.original:
        ok = run_sequence(
            ("structure", "link-prediction", "robustness", "statistics-original", "figures-structure", "figures-results"),
            args.continue_on_error,
        )
        needs_check = False
        ...
        if ok or args.continue_on_error:
            ok = run_sequence(("figures-structure", "figures-results", "figure-formats"), args.continue_on_error) and ok
        needs_check = True
    elif args.revision:
